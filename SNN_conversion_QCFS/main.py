...
if __name__ == "__main__":
    ...
    if args.gpus > 1:
        ...
        # preparing data
        train, test = datapool(args.data, args.bs, ee=args.mode == 'eesnn')
        # preparing model
        ...
        if args.action == 'train':
            model.load_state_dict(torch.load('./saved_models/' + 'c100_res20' + '.pth'))
            ...
            model.load_state_dict(torch.load('ImageNet-ResNet34-t8.pth'))
            if args.mode == 'snn':
                model = replace_activation_by_neuron(model)
                model.to(args.device)
                acc = eval_snn(test, model, args.device, args.t)
                print('Accuracy: ', acc)
            elif args.mode == 'eesnn':
                model = replace_activation_by_neuron(model)
                model.to(args.device)
                ee_test(model, test, 'cuda', args.t, threshold=args.thresh)
